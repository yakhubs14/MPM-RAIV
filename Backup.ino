/*
 * PROJECT MPM-RAIV - FIRMWARE v53.0 (DIR SWAP & SPEED SAFETY)
 * -----------------------------------------------------------
 * CHANGELOG:
 * - FIX 1 (Direction): Swapped FWD/BWD logic in ISR to match physical wiring.
 * Now Forward command moves vehicle Forward.
 * - FIX 2 (Speed Safety): Added logic to reduce speed by 30% if target is >= 1.0 m/s.
 * - CORE: Maintained ESP32 Core 3.0+ compatibility.
 */
...
void IRAM_ATTR onTimer() {
  ...
  if (isRunning) {
    // 1. Pulse Generation (SWAPPED LOGIC FOR DIRECTION FIX)
    if (isForward) {
       // Code says Forward, but physically needs "Backward" pins to move Forward
       digitalWrite(PIN_LB, HIGH); digitalWrite(PIN_RB, HIGH);
       for(volatile int i=0; i<50; i++); 
       digitalWrite(PIN_LB, LOW); digitalWrite(PIN_RB, LOW);
    } else {
       // Code says Backward, physically needs "Forward" pins
...
// Function to update Timer Frequency
void setSpeed(int delayUs) {
    if (delayUs < 500) delayUs = 500; 
    // ESP32 Core 3.0: Update alarm value
    timerAlarm(timer, delayUs, true, 0); 
}
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n--- RAIV v53.0 (DIR FIX & SPEED SAFETY) ---");
...
void loop() {
  ...
  if (signupOK && !isManualHold && isRunning && millis() - lastSpeedPoll > 1000) {
      if (Firebase.RTDB.getInt(&fbDO, "/speed_factor")) {
          ...
          // SAFETY REDUCTION LOGIC
          // If speed is >= 1.0 m/s, reduce by 30% (multiply by 0.7)
          if (selectedSpeedMs >= 1.0) {
               selectedSpeedMs = selectedSpeedMs * 0.70;
          }

          // Expert Curve (Applied AFTER safety reduction)
